    ...
    match result {
        ...
        Err(mut error) => {
            ...
            if let Some(launch) = &launched
                && let Err(termination_error) = hooks.terminate_codex(launch).await
            {
                error = anyhow::anyhow!(
...
impl LaunchHooks for DefaultLaunchHooks {
    ...
    async fn apply_active_relay_profile(
            ...
            && !profile.official_mix_api_key
        {
            ...
            crate::relay_config::clear_relay_config_to_home_with_auth(codex_home, auth_contents)?;
            return Ok(());
        }
    ...
    async fn wait_for_codex_exit(
        ...
        loop {
            ...
            let cdp_available = should_probe_launcher_cdp(cfg!(windows), has_codex_process)
                && crate::cdp::endpoint_available(debug_port);
            // 进程列表和 CDP 端口任一还在就算活着；连续观察到都不在才判定退出，
            // 兼顾 Linux 启动期进程短暂消失与已改用 CDP 探测的其它平台。
            if exit_observation.observe(launcher_target_alive(has_codex_process, cdp_available)) {
                break;
            }
...
async fn quit_macos_app_and_wait(app_dir: &Path) -> anyhow::Result<()> {
    ...
    while is_macos_app_running(app_dir).await.unwrap_or(false) {
