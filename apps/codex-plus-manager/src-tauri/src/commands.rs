...
mod tests {
    ...
    fn provider_sync_home_fixture() -> (tempfile::TempDir, TestHomeEnvGuard) {
        ...
        // 上游的供应商同步要求 model_provider 能精确对应到一张 live 的 model_providers 表。
        std::fs::write(
            codex_home.join("config.toml"),
            "model_provider = \"codex_home_test\"\n\n[model_providers.codex_home_test]\nname = \"codex_home_test\"\nbase_url = \"https://codex-home.example/v1\"\nwire_api = \"responses\"\n",
        )
        .unwrap();
        std::fs::write(
            default_codex_home.join("config.toml"),
            "model_provider = \"default_home_test\"\n\n[model_providers.default_home_test]\nname = \"default_home_test\"\nbase_url = \"https://default-home.example/v1\"\nwire_api = \"responses\"\n",
        )
        .unwrap();
    ...
    fn provider_sync_command_uses_valid_codex_home_instead_of_default_user_home() {
        ...
        assert_eq!(sync.status, "ok", "{}", sync.message);
